...
const initRegistrationForm = () => {
  ...
  const heroBtn = document.getElementById('hero-survey-claim');
...
if (survey) {
  ...
  const steps = document.querySelectorAll('.hero-survey__step');
  const checking = document.querySelector('.hero-survey__checking');
  const finalBlock = document.querySelector('.hero-survey__final');
  const progress = document.querySelector('.hero-survey__progress span');

  let currentStep = 0;

  if (localStorage.getItem('surveyFinished') === 'true') {
    steps.forEach(step => {
  ...
  document.querySelectorAll('.hero-survey__btn').forEach(button => {
